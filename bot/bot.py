...
⚪ /retry – Обновление последнего ответа бота
⚪ /new – Начать новый диалог
⚪ /mode – Выбрать модель чата
⚪ /settings – Настройки
⚪ /balance – Баланс
⚪ /help – Помощь
"""
...
async def start_handle(update: Update, context: CallbackContext):
    ...
    reply_text = "Привет! я <b>ChatGPT</b> бот созданный с GPT-3.5 OpenAI API 🤖\n\n"
    reply_text += HELP_MESSAGE

    reply_text += "\nА сейчас... спроси у меня что-нибудь!"
    ...
    async def message_handle_fn():
        ...
        if use_new_dialog_timeout:
            if (datetime.now() - db.get_user_attribute(user_id, "last_interaction")).seconds > config.new_dialog_timeout and len(db.get_dialog_messages(user_id)) > 0:
                db.start_new_dialog(user_id)
                await update.message.reply_text(f"Запуск нового диалога из-за тайм-аута (<b>{openai_utils.CHAT_MODES[chat_mode]['name']}</b> mode) ✅", parse_mode=ParseMode.HTML)
        db.set_user_attribute(user_id, "last_interaction", datetime.now())
...
async def is_previous_message_not_answered_yet(update: Update, context: CallbackContext):
    ...
    if user_semaphores[user_id].locked():
        text = "⏳ Пожалуйста, <b>ждите</b> ответа на предыдущее сообщение\n"
...
async def post_init(application: Application):
    await application.bot.set_my_commands([
        ...
        BotCommand("/mode", "Выбрать модель чата"),
        BotCommand("/retry", "Обновление последнего ответа бота"),
